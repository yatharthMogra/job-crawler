...
class _DummySession:
    def __init__(self, *, domain_rows: list[SimpleNamespace], title_rows: list[SimpleNamespace]) -> None:
    ...
    async def execute(self, _stmt):
        ...
        if self._call_count == 1:
            return _ExecuteResult(self._domain_rows)
...
def test_taxonomy_health_top_titles_limited_to_ten() -> None:
    domain_rows = [SimpleNamespace(domain="Software", total=20, no_pool=20)]
    title_rows = [
